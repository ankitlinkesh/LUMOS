...
There is no login layer: every route is reachable with no session, and the
tenant/defense values in a request come straight from the caller. See
``triad/api/app.py``'s module docstring for the integration contract this
implies for anything that puts this service behind untrusted callers.
"""
...
def make_client() -> TestClient:
    return TestClient(create_app(FakeDemoService()))
...
def test_meta_shape_and_fake_service_reports_fake():
    ...
    # valid value" -- this is the flag a caller keys off to know whether
    # anything it just received is a real measurement.
...
def test_ask_defense_off_is_the_leaky_baseline():
    # With no login layer, `defense` comes straight from the caller. This is
    # the deliberately vulnerable path -- FakeDemoService's own canned
    # scenario -- and it is reachable exactly as asked; see app.py's
    # docstring for why a caller exposing this to end users must control who
    # can ever set defense=false.
    client = make_client()
...
def test_probe_shape_secure_vs_leaky():
    ...
    # what lets a caller relabel the number instead of quoting it as a real
    # measurement (the real adapter reports fake=False once wired).
